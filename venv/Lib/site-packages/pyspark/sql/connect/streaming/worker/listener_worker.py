...
Usually this is run on the driver side of the Spark Connect Server.
"""
import os
import json
...
def main(infile: IO, outfile: IO) -> None:
    ...
    if connect_url.lower().startswith("unix"):  # Used in process-isolation mode.
        builder = UDSChannelBuilder(connect_url)
        builder.set("x-databricks-local-credentials-key", credentials_key)
        builder.set("session_id", session_id)
        spark_connect_session = SparkSession.builder.channelBuilder(builder).getOrCreate()
    else:
