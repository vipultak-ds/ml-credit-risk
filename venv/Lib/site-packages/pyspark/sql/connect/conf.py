...
class RuntimeConf:
    ...
    # BEGIN-EDGE
    def _allow_confs(self, f):
        """Allow to set any configuration. This is for testing only."""
        origin = self.get("spark.testing.databricks.runtimeConfigAllowlist.enabled")
        self.set("spark.testing.databricks.runtimeConfigAllowlist.enabled", False)
        try:
            f()
        finally:
            self.set("spark.testing.databricks.runtimeConfigAllowlist.enabled", origin)
